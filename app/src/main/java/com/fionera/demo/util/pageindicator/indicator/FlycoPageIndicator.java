package com.fionera.demo.util.pageindicator.indicator;
...
public class FlycoPageIndicator
    ...
    public void onPageSelected(int position) {
        if (isSnap) {
            ...
            for (int i = 0; i < indicatorViews.size(); i++) {
                indicatorViews.get(i)
                        .setImageDrawable(i == position ? selectDrawable : unSelectDrawable);
            }
            animSwitch(position);
            lastItem = position;
        }
            ...
            if (selectAnimClass != null) {
                if (position == lastItem) {
                    ...
                    if (unselectAnimClass == null) {
                        selectAnimClass.newInstance().interpolator(new ReverseInterpolator())
                                .playOn(indicatorViews.get(lastItem));
                    } else {
    ...
    private void createIndicators() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            iv.setImageDrawable(
                    !isSnap ? unSelectDrawable : (currentItem == i ? selectDrawable :
                            unSelectDrawable));
