...
//            //获取模板的X轴缩放级别，两者做比较
//            return scale != mScale;
            return totalScale == 1.0f ? false : true;
        }
        ...
        private float checkDyBound(float[] values, float dy) {
            ...
            Log.e("--values--", mCurrentMatrix.toString());
        ...
        private void setZoomMatrix(MotionEvent event) {
            ...
            if (endDis > 10f) { // 两个手指并拢在一起的时候像素大于10
                ...
                centerF = getCenter(scale, values);
                mCurrentMatrix.postScale(scale, scale, centerF.x, centerF.y);

                totalScale = totalScale * scale;
                Log.e("--totalScale--", "" + totalScale);

                float rotation = rotation(event) - oldRotation;
                oldRotation = rotation(event);
                mCurrentMatrix.postRotate(rotation, centerF.x, centerF.y);
...
//                setImageMatrix(mCurrentMatrix);
                int count = 0;
                if (((int) totalRotation) % 90 >= 45) {
                    count = 1;
                }
                if (((int) totalRotation) % 90 <= -45) {
                    count = -1;
                }
                float rotate = ((((int) totalRotation) / 45 + count) * 45);
                mCurrentMatrix.postRotate(rotate, getWidth() / 2, getHeight() / 2);
                totalRotation = rotate;
                setImageMatrix(mCurrentMatrix);

                Log.e("----", mCurrentMatrix.toString());
            } else {
                //判断Y轴是否需要更正
//                float[] values = new float[9];
//                getImageMatrix().getValues(values);
...
//                }
                ...
                Log.e("--rotate--", "" + rotate);
                if (centerF != null) {
                    mCurrentMatrix.postScale(totalScale, totalScale, centerF.x, centerF.y);
                    mCurrentMatrix.postRotate(rotate, centerF.x, centerF.y);
                }
...
//            //获取模板的X轴缩放级别，两者做比较
//
//            Log.e("Matrix", "values[Matrix.MSCALE_X] : " + values[Matrix.MSCALE_X]);
//            Log.e("Matrix", "values[Matrix.MSCALE_Y] : " + values[Matrix.MSCALE_Y]);
//            Log.e("Matrix", "values[Matrix.MPERSP_0] : " + values[Matrix.MPERSP_0]);
//            Log.e("Matrix", "values[Matrix.MPERSP_1] : " + values[Matrix.MPERSP_1]);
//            Log.e("Matrix", "values[Matrix.MPERSP_2] : " + values[Matrix.MPERSP_2]);
//            Log.e("Matrix", "values[Matrix.MSKEW_X] : " + values[Matrix.MSKEW_X]);
//            Log.e("Matrix", "values[Matrix.MSKEW_Y] : " + values[Matrix.MSKEW_Y]);
//            Log.e("Matrix", "values[Matrix.MTRANS_X] : " + values[Matrix.MTRANS_X]);
//            Log.e("Matrix", "values[Matrix.MTRANS_Y] : " + values[Matrix.MTRANS_Y]);
//            return scale < mScale;
            return totalScale > 1 ? false : true;
        }
